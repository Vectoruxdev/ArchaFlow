...
const handler: TriggerHandler = {
  type: 'card_tag_added',
  label: 'Card tag added',
  description: 'Triggers when a specific tag is added to a card',
  icon: 'Tag',
  configSchema: {
    fields: [
      {
  ...
  matches(event: KanbanEvent, config: Record<string, unknown>): boolean {
    if (event.type !== 'card_tag_added') return false
    if (config.tag) {
  ...
  summarize(config) {
    return `tag "${config.tag ?? '...'}" is added to a card`
  },
}
